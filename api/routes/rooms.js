...
const dateAEST = date => {
  return momentTimezone(date).tz('Asia/Hong_Kong')
}
...
router.put('/rooms/:id', requireJWT, (req, res) => {
  ...
  // If the booking is a recurring booking
    ...
    let bookingDateTracker = momentTimezone(firstBooking.bookingStart).tz('Asia/Hong_Kong')
    ...
    let lastBookingDate = momentTimezone(firstBooking.recurring[0]).tz('Asia/Hong_Kong')
    ...
    for (let i = 0; i < bookingsInRange; i++) {
      ...
      if (proposedBookingDateStart.day() !== 0) {
        ...
        let firstBookingEndDate = momentTimezone(firstBooking.bookingEnd).tz('Asia/Hong_Kong')
